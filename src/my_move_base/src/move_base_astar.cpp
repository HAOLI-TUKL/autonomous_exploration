...
void InitPubMarkerarray_report(vector<A_star::cell> path, int count);
A_star astar;
...
//                sent = false;// in order to publish finished
            ...
            InitPubMarkerarray_report(astar.path,count_++);
...
void PublishCmd(){
    ...
    if (res[2] == 1.0){// if car reached the current goal
	    ...
	    sum_time += double(diff.count())* microseconds::period::num / microseconds::period::den;
	cout<<" end time !!!!!!!!!!!!!!!!!!!!"<<double(diff.count())* microseconds::period::num / microseconds::period::den<<"  "<<sum_time<<endl;
        }
...
void InitPubMarkerarray_report(vector<A_star::cell> path, int count){
    // clear all the marker
        ...
        for (int j = 0; j < path.size(); ++j) {
            ...
            marker.scale.z = 0.1;
            marker.color.r = 0.0f ;
            marker.color.g = 1.0f * (1 - count / 7.0) ;
            marker.color.b = 1.0f * ( count / 7.0);
            marker.color.a = 1;
            marker.lifetime = ros::Duration();
            ma.markers.push_back(marker);
        }
