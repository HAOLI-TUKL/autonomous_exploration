//
// Created by parallels on 4/21/20.
//
#include "ros/ros.h"
...
void Initmarker(HybricAStar::cell cell ,int id);//for test
void InitPubMarkerarray(vector<HybricAStar::cell> path);//for test
...
int main(int argc, char **argv){
    ros::init(argc, argv, "move_base_hybri");
    ros::NodeHandle n;
    ...
    while(ros::ok())
    {
        ...
        if(newfrontierreceived == true){// search only when new frontier comes
            ...
            Hastar.initialize();
            Initmarker(Hastar.starting_point,10001);//
            Initmarker(Hastar.goal,10002);
            Hastar.A_star_search();
            closelistmap.publish(Hastar.close_list_map_);
            if(Hastar.search_success_ == false){
                PublishNoPathFound();
            }else{
                for(int i = 0;i < Hastar.path.size();i++){
                    cout<<" vel of each point : "<< Hastar.path[i].vs<<endl;
                }
            }
...
void InitStartingGoalPoint(){
    ...
    double theta_s = GetThetaFromquaternion(currentposori[1]);
    Hastar.SetStartingGoalPoint(coordx,coordy ,theta_s,goalxcoord,goalycoord);
...
bool PointInObstacle(double coordx,double coordy){// true means point in obstacle
    ...
    vector<int> gridxy =Hastar.coord_to_grid(coordxy);
...
void PublishCmd(){
    ...
    if(Hastar.search_success_ == false){ return;}
    sent = false;// once start following, reset
    HybricAStar::cell& goal_cell = Hastar.path.back();//first element of path is the final goal
    ...
    if (res[2] == 1.0){// if car reached the current goal
        Hastar.path.pop_back();
        if(Hastar.path.size() == 0){
            geometry_msgs::Twist msg;
            msg.linear.x = 0;
            msg.linear.y = 0;
            msg.linear.z = 0;
