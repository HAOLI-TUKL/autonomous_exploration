...
void Initmarker(RRT::cell cell ,int id);//for test
void InitPubMarkerarray(vector<RRT::cell> path);//for test
...
int main(int argc, char **argv){
    ros::init(argc, argv, "move_base_rrt");
    ros::NodeHandle n;
    ...
    newmappub = n.advertise<nav_msgs::OccupancyGrid>("close_list_map", 1000);//
    ...
    while(ros::ok())
    {
        ...
        if(newfrontierreceived == true){// search only when new frontier comes
            ...
            builder.inflation();
            rrt.my_map = builder.inflation_map;
            pubinflationmap.publish(builder.inflation_map);
            InitStartingGoalPoint();
            Initmarker(rrt.starting_point,10001);//
            Initmarker(rrt.goal,10002);
            rrt.build_tree();
            newmappub.publish(rrt.new_map);
            if(rrt.search_success_ == false){
                PublishNoPathFound();
            }else{
                for(int i = 0 ; i < rrt.path.size() ; i++){
                    int gridXY[2]= {rrt.path[i].x,rrt.path[i].y};
                    vector<double> coordXY = rrt.grid_to_coord(gridXY);
                    cout<<" path "<<i<<" : "<<coordXY[0]<<","<<coordXY[1]<<endl;
                }
            }
            InitPubMarkerarray(rrt.path);
...
void InitStartingGoalPoint(){
    ...
    rrt.SetStartingGoalPoint(coordx,coordy ,goalxcoord,goalycoord);
...
bool PointInObstacle(double coordx,double coordy){// true means point in obstacle
    ...
    vector<int> gridxy =rrt.coord_to_grid(coordxy);
...
void PublishCmd(){
    ...
    if(rrt.search_success_ == false){ return;}
    sent = false;// once start following, reset
    RRT::cell& goal_cell = rrt.path.back();//first element of path is the final goal
...
void Initmarker(RRT::cell cell ,int id){
    int gridXY[2] = {cell.x , cell.y};
    std::vector<double > mapXY;
    mapXY = rrt.grid_to_coord(gridXY);
...
void InitPubMarkerarray(vector<RRT::cell> path){
    ...
    if(path.size() == 0){
        ...
        for (int j = 0; j < path.size(); ++j) {
            int  gridXY[2] = {int((path.at(j).index + rrt.my_map.info.width)%rrt.my_map.info.width),
                              int(floor(path.at(j).index/rrt.my_map.info.width))};
            std::vector<double > mapXY;
            mapXY = rrt.grid_to_coord(gridXY);
            visualization_msgs::Marker marker;
            marker.header.frame_id = "map";
