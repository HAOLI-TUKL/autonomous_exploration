...
void Initmarker(RRT_STAR::cell cell ,int id);//for test
void InitPubMarkerarray(vector<RRT_STAR::cell> path);//for test
 ...
 loop_rate =  new ros::Rate(50);
    ...
    while(ros::ok())
    {
        ...
        if(newfrontierreceived == true){// search only when new frontier comes
            ...
            Initmarker(rrt_a.starting_point,10001);//
            Initmarker(rrt_a.goal,10002);
            rrt_a.build_tree();
            newmappub.publish(rrt_a.new_map);
            if(rrt_a.search_success_ == false){
                PublishNoPathFound();
            }else{
                for(int i = 0 ; i < rrt_a.path.size() ; i++){
                    int gridXY[2]= {rrt_a.path[i].x,rrt_a.path[i].y};
                    vector<double> coordXY = rrt_a.grid_to_coord(gridXY);
...
void InitStartingGoalPoint(){
    ...
    rrt_a.SetStartingGoalPoint(coordx,coordy ,goalxcoord,goalycoord);
...
bool PointInObstacle(double coordx,double coordy){// true means point in obstacle
    ...
    vector<int> gridxy =rrt_a.coord_to_grid(coordxy);
...
void PublishCmd(){
    ...
    if(rrt_a.search_success_ == false){ return;}
    sent = false;// once start following, reset
    RRT_STAR::cell& goal_cell = rrt_a.path.back();//first element of path is the final goal
