...
void myCallback_goal_rviz(geometry_msgs::PoseStamped goal){
    if(type_from_rviz == 0){
        ...
        vector<int> gridXY;
        gridXY = rrt.coord_to_grid(coord);
        rrt.starting_point = *new RRT::cell(gridXY[0],gridXY[1],gridXY[1]*rrt.my_map.info.width+gridXY[0],coord[0],coord[1],0,0) ;//8153962 8217882 shit:8217882 x:1882 y:2054        astar.starting_point.g = 0;
        ...
        vector<int> gridXY;
        gridXY = rrt.coord_to_grid(coord);
        rrt.goal = *new RRT::cell(gridXY[0],gridXY[1],gridXY[1]*rrt.my_map.info.width+gridXY[0],coord[0],coord[1],-1,-1) ;//8153962 8217882 shit:8217882 x:1882 y:2054
...
//        Hastar.set_start_end();
        ...
        if(receive_goal_from_rviz==true &&receive_starting_from_rviz==true) {
            ...
            Initmarker(rrt.starting_point,100);
            Initmarker(rrt.goal,2000);
            pub_inf.publish(rrt.new_map);
            cout << " size of path :" << rrt.path.size() << endl;
