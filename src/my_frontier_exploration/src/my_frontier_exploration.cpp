...
void init_marker_report_2(frontier_search::frontier regions, int id){
    int gridXY[2] = {regions.centroid.x , regions.centroid.y};
    std::vector<double > mapXY;
    mapXY = frontierSearch->grid_to_coord(gridXY);
    visualization_msgs::Marker marker;
    marker.header.frame_id = "map";
...
void init_pub_markerarray_report(vector<frontier_search::cell> cells){

        visualization_msgs::MarkerArray ma;
        for (int j = 0; j < cells.size(); ++j) {
            int gridXY[2] = {cells[j].x , cells[j].y};
            std::vector<double > mapXY;
            mapXY = frontierSearch->grid_to_coord(gridXY);
            visualization_msgs::Marker marker;
            marker.header.frame_id = "map";
...
void init_pub_markerarray_report_2(vector<frontier_search::frontier> regions){

        visualization_msgs::MarkerArray ma;
	float size = regions.size();
	for(int i = 0; i < regions.size(); i++){
        for (int j = 0; j < regions[i].vector.size(); ++j) {
            int gridXY[2] = {regions[i].vector[j].x , regions[i].vector[j].y};
            std::vector<double > mapXY;
            mapXY = frontierSearch->grid_to_coord(gridXY);
            visualization_msgs::Marker marker;
            marker.header.frame_id = "map";
            marker.header.stamp = ros::Time::now();
            marker.id = 1000*i + j;//avoid to have same ids with the marker of path generated by move_base
            marker.type = visualization_msgs::Marker::CUBE;
            marker.action = visualization_msgs::Marker::ADD;
            marker.pose.position.x = mapXY[0];
            marker.pose.position.y = mapXY[1];
            ...
            marker.scale.y = 0.1;
            marker.scale.z = 0.1;
            marker.color.r = 1.0f * i /size;
            marker.color.g = 1.0f * i/  (size - i) ;
            marker.color.b = 0.0f;
            marker.color.a = 1.0;
            marker.lifetime = ros::Duration();
            ma.markers.push_back(marker);
...
/*	////////////////////// report
	...
	pub.publish(builder.inflation_map);
        ...
        frontierSearch->frontier_regions = frontierSearch->get_frontier_regions(
                frontierSearch->frontier_edge_cells);
	frontierSearch->frontiers = frontierSearch->get_frontiers(frontierSearch->frontier_regions);
	init_pub_markerarray_report_2(frontierSearch->frontiers);

	int idd = 0;
	for(auto i : frontierSearch->frontiers) init_marker_report_2(i,idd++);
	exit(0);
	//////////////////////report*/
    }
    cout<<"###################### map end "<<endl;
...
int main(int argc, char **argv)
{
    ...
    pub = n.advertise<nav_msgs::OccupancyGrid>("my_inflation_map", 1000);
